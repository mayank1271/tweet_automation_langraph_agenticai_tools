# ✅ SCRIPT 1: Tweet Fetcher & Scheduler (Auto-login, x.com version)
import time
import re
import schedule
import pandas as pd
...
async def login_x_with_cookies(context, page):
    """Login automatically using saved cookies. Refresh if invalid."""
    if COOKIE_PATH.exists():
        try:
            ...
            if "login" not in page.url.lower():
                ...
                print("⚠️ Cookies expired. Need manual refresh once.")
        except Exception as e:
            print(f"Cookie read error: {e}")

    # If login fails, ask to login manually (only once)
    print("⚠️ Please login manually once on X.com (window opened)...")
    await page.goto("https://x.com/login", timeout=60000)
    await asyncio.sleep(30)  # Give time to login manually
...
def build_search_url(user_id, since_datetime, until_datetime):
    ...
    return f"https://x.com/search?q=%40{user_id}%20since%3A{since_str}%20until%3A{until_str}&src=typed_query&f=live"
...
async def scrape_and_save():
    ...
    async with async_playwright() as p:
        ...
        await login_x_with_cookies(context, page)
