...
# === CONFIG ===
COOKIES_JSON_PATH = Path(r"C:\Users\mayank manjhi\Downloads\(1)lang,rag,ai\x_cookies.json")  # manually copied cookies
IST = pytz.timezone("Asia/Kolkata")
os.makedirs("railways/images", exist_ok=True)
os.makedirs("railways/videos", exist_ok=True)
SCREENSHOT_DIR = Path("railways/screenshots")
...
async def load_cookies(context):
    ...
    # convert string expires to int timestamp
    for c in cookies:
        if "expires" in c and isinstance(c["expires"], str):
            try:
                ...
                print(f"⚠️ Failed to parse expires for {c.get('name')}: {e}")
                c.pop("expires", None)
...
async def ensure_login(page):
    try:
        await page.goto("https://x.com/home", timeout=4000000)
        await asyncio.sleep(3)
        if "login" in page.url.lower():
            screenshot_path = SCREENSHOT_DIR / f"login_failed_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
            await page.screenshot(path=str(screenshot_path), full_page=True)
            raise RuntimeError(f"❌ Not logged in. Cookies may be expired. Screenshot saved: {screenshot_path}")
        ...
        print("⚠️ Login check timeout, continuing (assuming valid session).")
    ...
    async with async_playwright() as p:
        ...
        for uid in user_ids:
            ...
            try:
                ...
                print(f"⚠️ Unexpected error for @{uid}: {e}")
