...
async def repost_node(state: AgentState):
    ...
    await repost_tweet.ainvoke({"tweet_url": state["tweet_url"]})  # repost the tweet
    return {**state}
...
async def comment_node(state: AgentState):
    ...
    if not state.get("comment_text"):
        return state

    try:
        # Properly call BaseTool using ainvoke
        await comment_tweet.ainvoke({
            "tweet_url": state["tweet_url"],
            "comment_text": state["comment_text"]
        })
        print("💬 Comment executed successfully via LangChain Tool.")
